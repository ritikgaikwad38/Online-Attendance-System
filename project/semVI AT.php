...
<div class="wrapper">
    ...
    <form method="post" action="">
		...
		<select name="Subject" class="Subject" required="value">
			...
			<option value="SPCC">System Programming & Compiler Construction</option>
			<option value="DWM">Data Warehousing & Mining</option>
			<option value="CSS">Cryptography & Sytem Security</option>
		</select>
...
$DBname = "yogesh";
 ...
 if($con)
 {
	...
	{
		$sql = "CREATE TABLE IF NOT EXISTS semVI_AT(Roll_no CHAR(8),Subject CHAR(10),Type VARCHAR(10),date CHAR(13),Lecture CHAR(3),First CHAR(6), Second CHAR(6),Attendance CHAR(3))";
 		if (mysqli_query($con, $sql))  
 		{
 			...
 			$From=$_POST['From'];
 			$To=$_POST['To'];
 			$sql = "SELECT * FROM semVI_AT WHERE Roll_no='$Roll_no' AND Subject='$Subject' AND date='$Date' AND First='$From' AND Second='$To'";
			...
			if($Attendance=='present')
			{
				...
				if ($hours_difference==2) 
				{
					...
					{
						$Attendance=2;
						$sql="INSERT INTO semVI_AT(Roll_no,Lecture,Attendance,Subject,Type,First,Second,date)VALUES('$Roll_no','$Lecture','$Attendance','$Subject','$Type','$From','$To','$date')"; 	
