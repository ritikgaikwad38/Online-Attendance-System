...
<div class="wrapper">
    ...
    <form method="post" action="">
		...
		<select name="Subject" class="Subject" required="value">
			...
			<option value='DBMS'>DATABSE MANAGEMENT SYSTEM</option>
			<option value='WDL'>WEB DEVELOPMENT</option>
			<option value='MP'>MICROPROSESSOR</option>
			<option value='TCS'>THEORY OF COMPUTER SCIENCE</option>
			<option value='MSD'>MULTIMEDIA SYSTEM</option>
			<option value='BCE'>BUISNESS COMMUNICATION AND ETHICS</option>
		</select>
...
$DBname = "yogesh";
 ...
 if($con)
 {
	...
	{
		$sql = "CREATE TABLE IF NOT EXISTS semV_AT(Roll_no CHAR(8),Subject CHAR(10),Type VARCHAR(10),date CHAR(13),Lecture CHAR(3),First CHAR(6), Second CHAR(6),Attendance CHAR(3))";
 		if (mysqli_query($con, $sql))  
 		{
 			...
 			$From=$_POST['From'];
 			$To=$_POST['To'];
 			$sql = "SELECT * FROM semV_AT WHERE Roll_no='$Roll_no' AND Subject='$Subject' AND date='$Date' AND First='$From' AND Second='$To'";
			...
			if ($Subject=='TCS' && $Type=='PRACTICAL')
			{
				echo "<script>alert('TCS Have no Practicals');</script>";	
			}
			else if($Subject=='MSD' && $Type=='PRACTICAL')
			{
				echo "<script>alert('MSD Have no Practicals');</script>";
			}
			else if($Subject=='WDL' && $Type=='THEORY')
			{
				echo "<script>alert('WDL Have no Theory');</script>";
			}
			else
			{
			$From=$_POST['From'];
			...
			if($Attendance=='present')
			{
				...
				if ($hours_difference==2) 
				{
					...
					{
						$Attendance=2;
						$sql="INSERT INTO semV_AT(Roll_no,Lecture,Attendance,Subject,Type,First,Second,date)VALUES('$Roll_no','$Lecture','$Attendance','$Subject','$Type','$From','$To','$date')"; 	
